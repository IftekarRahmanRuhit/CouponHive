  ...
  return (
    <div>
      <p className="bg-slate-50 w-11/12 mx-auto text-center text-xl font-bold pt-8 pb-7 text-[#E1713B] animate__animated animate__backInRight">🔥 Sale is On! Grab Your Deals Now!</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-6 w-11/12 mx-auto bg-slate-50 rounded-lg ">
      {saleBrands.length > 0 ? (
        saleBrands.map((brand) => (
          <Link to='/brands' key={brand.id} className="bg-white shadow-md rounded-lg overflow-hidden border p-3 transition hover:scale-105 overflow-hidden">
            ...
            <div className="p-4 text-center">
              ...
              <p className="text-gray-600 mt-1">{brand.category}</p>
              <p className="text-gray-500 mt-1">Total Coupons: {brand.total_coupons}</p>
            </div>
          </Link>
        ))
      ) : (
